...
- [ ] 備註風險：目前 `ApiResult.Exception` 完整回傳堆疊資訊，日後若轉正式環境，建議加上環境切換策略。
...
- [ ] 補更完整的 API integration tests，實際驗證 HTTP 401/403/validation/500 response body。
- [ ] 評估是否導入 `/api/v1/...`，本輪依決策保留目前 `/api/...` 路由。