...
### planning — 專案規劃與進度追蹤
- [00-腳手架強化路線圖](planning/00-腳手架強化路線圖.md) ← **現行活文件**
- 以下 `01`～`06` 為 0.4.x 規劃階段的已封存快照，不再維護：
...
- [新增「腳手架開發指引」，並補上色票守門的漏洞（0.9.34）](changelog/2026-09-19-腳手架開發指引.md)
- [開發指引擴寫為技術與參數百科，並補上兩道守門（0.9.35）](changelog/2026-09-19-開發指引擴寫為技術與參數百科.md)
- [模型名稱不帶日期後綴：查證與約束文件化（0.9.36）](changelog/2026-09-19-模型名稱不帶日期.md)
- [權限控管設計指南，以及兩個讓動作級授權名不副實的缺陷（0.9.37）](changelog/2026-09-19-權限控管設計指南.md)
- [補上 Cookie 效期與滑動續期的完整說明（0.9.38）](changelog/2026-09-19-Cookie效期與滑動續期.md)
- [修好「登入存不住」：Data Protection 金鑰、強制登出放大器、Cookie 效期外部化（0.9.39）](changelog/2026-09-19-登入存不住修正.md)
- [側邊欄展開後的黑字與藍字：AntDesign 蓋過了深梅配色（0.9.40）](changelog/2026-09-19-側邊欄選單文字對比修正.md)
- [未登入也看得到頁面：改由 HTTP 層擋下，並補上權限閘門與 ReturnUrl 防護（0.9.41）](changelog/2026-09-19-未登入頁面閃現修正.md)
- [新增「稽核紀錄」查詢頁：稽核軌跡終於拿得出來（0.9.42）](changelog/2026-09-20-稽核紀錄查詢頁.md)
- [衍生新專案時自動清掉腳手架自身的歷史文件（0.9.46）](changelog/2026-09-21-衍生專案文件清理.md)
- [腳本警告訊息的判讀說明，以及最後一行的亂碼修正（0.9.47）](changelog/2026-09-21-腳本警告說明與BOM修正.md)
- [修掉三項「防線存在但不生效」的缺陷（0.9.48）](changelog/2026-09-21-三項防線失效修正.md)
- [費率表補上新旗艦 `gpt-6-astra`（0.9.49）](changelog/2026-09-22-新增GPT-6-Astra定價.md)
- [Token 用量頁加上「最近 N 天」摘要卡與每日費用趨勢（0.9.50）](changelog/2026-09-23-Token費用統計.md)
- [Token 報表：單一頁籤匯出，並把「每日趨勢」補進 PDF（0.9.51）](changelog/2026-09-23-Token報表單頁籤匯出.md)
- [PDF 的中文終於會換行了：溢出頁面與掉字修正（0.9.52）](changelog/2026-09-23-PDF中文斷行修正.md)
- [「篩選範圍合計」改為儀表板卡片（0.9.53）](changelog/2026-09-23-合計改為儀表板卡片.md)
- [補上 GPT-6 Sol 與 Luna 計費設定（0.9.54）](changelog/2026-09-23-新增GPT-6-Sol與Luna定價.md)
- [Token 用量頁新增「趨勢圖」頁籤：token 與費用的折線趨勢（0.9.55）](changelog/2026-09-23-Token趨勢折線圖.md)
...
- [忘記密碼與重設密碼：Email 連結自助重設、三頁共用登入外觀（0.9.60）](changelog/2026-09-24-忘記密碼與重設密碼.md)
- [開發環境預設開啟 Pickup 寄信，本機一啟動就能測忘記密碼（0.9.61）](changelog/2026-09-24-開發環境預設開啟Pickup寄信.md)
- [開發環境取消預設 Pickup 寄信，回到與出貨相同的 None（0.9.62）](changelog/2026-09-24-開發環境取消預設Pickup寄信.md)
- [移除「設定 API 密碼」：Google 帳號只用於網頁登入（0.9.63）](changelog/2026-09-24-移除設定API密碼.md)
- [登入 Cookie 名稱依專案區分：同主機多站不再互相登出（0.9.64）](changelog/2026-09-24-登入Cookie名稱依專案區分.md)
