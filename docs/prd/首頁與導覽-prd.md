...
- 側邊選單（`NavMenu.razor` + `SidebarMenuNode`）：
  ...
  - 無任何可用項目時顯示「尚無可用選單」。
  - 現有結構（由上而下）：首頁、專案管理（專案項目）、資料定義（分類清單／團隊清單）、統計與分析（日誌檢視）、系統管理（使用者管理／角色管理）、登出。
...
- id→權限鍵對應（節錄）：1→`角色_首頁`「首頁」、2→`角色_專案管理`「專案管理功能」、21→`角色_專案項目`「專案項目」、3→`角色_系統管理`「系統管理功能」、31→`角色_使用者管理`「使用者管理」、32→`角色_角色管理`「角色管理」、5→`角色_資料定義`「資料定義管理功能」、51→`角色_分類清單`「分類清單」、52→`角色_團隊清單`「團隊清單」、6→`角色_統計與分析`「統計與分析功能」、61→`角色_日誌檢視`「日誌檢視」、4→`角色_登出`「登出」。
- **例外：統計與分析（6／61）為管理員專屬**。兩個權限鍵刻意未列入 `RolePermissionService.GetRoleListPermissionAllName()`，因此不會種出 `Permission` 資料列、角色矩陣不顯示、任何角色都無法被授予，只有 `CheckAccessPage` 的管理員短路能通過。詳見 [日誌檢視 PRD](日誌檢視-prd.md)。
