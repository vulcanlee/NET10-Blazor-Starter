...
- 側邊選單（`NavMenu.razor` + `SidebarMenuNode`）：
  ...
  - 現有結構（由上而下，0.9.21 起為三層）：首頁、專案管理（專案項目）、資料定義（分類清單／團隊清單）、
    系統管理〔統計與分析（系統健康監控／日誌檢視／日誌等級設定／系統例外紀錄／資料庫用量／Token 用量）、權限管理（使用者管理／角色管理）〕、登出。
...
- 右上角使用者選單（`MainLayout.razor`）：顯示目前使用者名稱與「管理員」標記，展開後含「變更密碼」「關於」「登出」三項（0.9.63 起移除「設定 API 密碼」）。
...
- id→權限鍵對應（節錄）：1→`角色_首頁`「首頁」、2→`角色_專案管理`「專案管理功能」、21→`角色_專案項目`「專案項目」、3→`角色_系統管理`「系統管理功能」（管理員專屬，不在矩陣）、6→`角色_統計與分析`「統計與分析功能」（同左）、65→`角色_系統健康監控`「系統健康監控」（同左）、61→`角色_日誌檢視`「日誌檢視」（同左）、63→`角色_日誌等級設定`「日誌等級設定」（同左）、33→`角色_系統例外紀錄`「系統例外紀錄」（同左）、62→`角色_資料庫用量`「資料庫用量」（同左）、64→`角色_Token用量`「Token 用量」（同左）、34→`角色_權限管理`「權限管理」（同左）、31→`角色_使用者管理`「使用者管理」（同左）、32→`角色_角色管理`「角色管理」（同左）、5→`角色_資料定義`「資料定義管理功能」、51→`角色_分類清單`「分類清單」、52→`角色_團隊清單`「團隊清單」、4→`角色_登出`「登出」。
- **例外：系統管理整支（3／6／65／61／63／33／62／64／34／31／32）共 11 個權限鍵為管理員專屬**。
  這 11 個鍵刻意未列入 `RolePermissionService.GetRoleListPermissionAllName()`，因此不會種出 `Permission` 資料列、
