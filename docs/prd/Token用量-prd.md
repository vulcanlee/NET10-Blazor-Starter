...
- 範圍：一套通用的用量記錄基礎設施（任何 LLM 呼叫點都能接）、六個統計頁籤、
...
合計列下方三張卡：**近 1 天（今天）／近 7 天／近 30 天**，各顯示 NT$、US$、呼叫次數與合計 token。
...
**每日趨勢**（0.9.50 起，預設頁籤）＋ 依使用者／依作業／依模型／依型別
...
範圍由 `TokenUsageReportScope` 表示（`All`／`Daily`／`Account`／`Operation`／`Model`／
